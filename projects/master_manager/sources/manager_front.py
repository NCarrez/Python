...
def setup_userWidget(username, password):
    ...
    widget.reveal.pressed.connect(
        lambda arg1=widget.password, arg2=widget.reveal: switch_password_print_mode(
            arg1, arg2
        )
    )
...
def build_centralwidget(widget):
    ...
    for account in account_list:
        user_widget_list.append(
            setup_userWidget(account.get_username(key), account.get_password(key))
        )
